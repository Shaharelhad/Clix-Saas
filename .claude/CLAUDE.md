...
- **Hebrew text in bash variables gets corrupted on Windows** — never pass Hebrew/UTF-8 strings through bash shell variables (`$VAR`) or `curl -d`. The Windows bash environment (Git Bash/MSYS2) mangles non-ASCII characters during variable expansion. Instead, use `node -e` with the Hebrew text inside a JavaScript template literal and make HTTP requests via Node.js `https` module directly. This ensures UTF-8 encoding is preserved end-to-end.
...
- **Before editing `_shared/llm-engine.ts` or any `_shared/*.ts`** — these modules are bundled into every edge function that imports them, so a removed/renamed export crashes that function at boot with `does not provide an export named '<X>'`. This has already caused two full-platform WhatsApp outages (commits `f7a030e` → `8f9fa61`, and `333ca14` → `0137749`). Before removing or renaming any exported symbol, grep all `supabase/functions/*/index.ts` for the name and update call sites in the same commit. Do NOT sync a `_shared/*.ts` file against a "deployed version" you didn't generate yourself — each edge function bundles its own copy at deploy time, so the deployed state differs per function and syncing against the wrong one regresses current code. If a sync is actually needed, reconcile forward (redeploy every consumer) rather than backward (delete exports).
- **Before every edge function deploy, run `node scripts/check-shared-exports.mjs`** — scans every `supabase/functions/*/index.ts` for imports from `_shared/*.ts` and verifies each imported name is actually exported. Exit code 1 on any mismatch. Catches the "broke boot by deleting a `_shared/` export" class of bug offline, before it hits production.
...
- **Per-Bot Model Selector** — "Bot model" dropdown in FlowSettingsModal writes `workflows.flow_json.settings.llmModel` (an OpenRouter slug; selectable catalog `LLM_MODEL_GROUPS` in `types/flow.ts`). Passed to `callLLMEngine` as `config.model` on all 3 main-reply paths (flow-webhook `callOpenLLM` self-fetch, inngest `callLLMForAgent` merge, flow-demo `callLLMFallback`). Empty = system default, which is `google/gemini-2.5-pro` as of 2026-06-07 (set in `_shared/llm-engine.ts`). `ruppinai@gmail.com` is pinned to `gemini-2.5-flash`.
