...
import { sendListMessage, type ListSection, type ListRow } from "../_shared/wa-messaging.ts";
import { buildMonthSections, buildDaySections } from "../_shared/calendar.ts";
void sendListMessage; // referenced for parity with flow-webhook; demo handles list locally
...
interface FlowNode {
  ...
  data: {
    ...
    // mor_ai_agent
    systemPrompt?: string;
    // list — dynamic WhatsApp list. sections come from a previously-set variable
    // OR are generated by a calendar source mode.
    listBody?: string;
...
// Coerce a variable's value into a sections array. Accepts:
//   - already-shaped sections: [{ title?, rows: [{ rowId, title, ... }, ...] }, ...]
//   - flat rows array: [{ rowId, title, ... }, ...]  (auto-wrapped into a single unnamed section)
// Returns [] when the input doesn't match either shape.
function normalizeListData(raw: unknown): ListSection[] {
  if (!Array.isArray(raw) || raw.length === 0) return [];
  const first = raw[0] as Record<string, unknown>;
  if (first && typeof first === "object" && Array.isArray(first.rows)) {
    // already sections-shaped
    return raw
  ...
  // flat rows array — wrap in one section
...
// Match a user's reply to a row across all sections.
// Tries exact title match (case-insensitive), then numeric (1-based index across all sections).
...
async function executeNodeDemo(
  ...
  if (node.type === "list") {
    ...
    if (shouldTranslate) {
      body = await translateMessage(body, flowLang, targetLang);
      if (header) header = await translateMessage(header, flowLang, targetLang);
      if (footer) footer = await translateMessage(footer, flowLang, targetLang);
      if (buttonText) buttonText = await translateMessage(buttonText, flowLang, targetLang);
    }

    // Build sections depending on listSource mode. The send/match path below
    // is identical regardless of source.
    const listSource = node.data.listSource || "from_variable";
    const calendarLocale: "en" | "he" = node.data.outputLanguage === "en" ? "en" : "he";
    ...
    if (listSource === "calendar_months") {
      ...
      const monthVar = (node.data.listMonthVariable || "").trim();
      const monthRowId = monthVar ? (variables[monthVar] || "") : "";
      sections = buildDaySections(monthRowId, calendarLocale);
    } else {
      const dataVar = (node.data.listDataVariable || "").trim();
      // Prefer the raw JSON shadow stored by api_call (__raw_<name>); fall back
      // to the user-facing variable for cases where the upstream node stored
    ...
    if (sections.length === 0) {
      // No data → emit a graceful fallback message and advance via default edge.
      const fallback = (node.data.outputLanguage || "he") === "he"
        ? "מצטערים, לא נמצאו אפשרויות זמינות."
        : "Sorry, no options are currently available.";
      responses.push({ type: "text", content: fallback });
      const next = findNextNode(flow, node.id);
      return { nextNodeId: next?.id || null, waitForInput: false };
    }
    // Emit a structured list response — the frontend (ChatBubble) renders
    // sections + tappable rows. No text-blob fallback needed: the preview chat
    // is the only consumer here, and it now supports the `list` type.
    responses.push({
      type: "list",
      content: body,
      ...(header ? { header } : {}),
  ...
  if (node.type === "api_call") {
    ...
    try {
      ...
      for (const mapping of responseMapping) {
        ...
        if (!isEmpty) {
          // For objects/arrays we LLM-format the user-facing variable but also
          // stash the raw JSON under __raw_<name> so structured nodes (e.g. list)
          // can consume the original shape. Backward-compatible: nothing else
          // reads __raw_* keys.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Handle user input for waiting nodes
      ...
      // re-read the variable (still in the session).
      const listSource = currentNode.data.listSource || "from_variable";
      const calendarLocale: "en" | "he" = currentNode.data.outputLanguage === "en" ? "en" : "he";
      let sections: ListSection[] = [];
      if (listSource === "calendar_months") {
        sections = buildMonthSections(calendarLocale);
      } else if (listSource === "calendar_days") {
        const monthVar = (currentNode.data.listMonthVariable || "").trim();
        const monthRowId = monthVar ? (variables[monthVar] || "") : "";
        sections = buildDaySections(monthRowId, calendarLocale);
      } else {
        const dataVar = (currentNode.data.listDataVariable || "").trim();
        const raw = dataVar
          ? (variables[`__raw_${dataVar}`] ?? variables[dataVar] ?? null)
          : null;
        let parsed: unknown = raw;
      ...
      const matched = matchRow(sections, message);
      if (matched) {
        const rowIdVar = (currentNode.data.listRowIdVariable || "").trim();
        const titleVar = (currentNode.data.listTitleVariable || "").trim();
        if (rowIdVar) variables[rowIdVar] = matched.rowId;
        if (titleVar) variables[titleVar] = matched.title;
        const nextNode = findNextNode(flow, currentNode.id);
        nextNodeId = nextNode?.id || null;
      } else {
        // No match — re-send the same list node.
        await executeNodeDemo(currentNode, variables, flow, responses);
        return new Response(
          JSON.stringify({ response: responses, conversation_id: convId, session_state: { current_node_id: currentNodeId, variables, status: "active" } }),
          { headers: { ...cors, "Content-Type": "application/json" } }
