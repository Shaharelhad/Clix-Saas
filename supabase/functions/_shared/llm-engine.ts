...
/**
 * Classify a user message into one of N intents using LLM.
 * Used by the AI Router node to route flow execution.
 */
export async function classifyIntent(
  ...
  const intentsJson = JSON.stringify(
    intents.map((i) => ({ id: i.id, label: i.label, description: i.description })),
  );

  const contextLine = context ? `\nAdditional context about the current state:\n${context}\n` : "";

  const systemPrompt = `You are a message intent classifier for a WhatsApp chatbot. Given a list of intents and a user message, determine which intent best matches the user's message.
Consider the intent descriptions carefully. Match based on the user's actual intent, not just keywords. Consider: Hebrew and English, casual WhatsApp language, typos, slang, and implied meaning.
${contextLine}
Available intents:
${intentsJson}

Respond with ONLY valid JSON, nothing else.
If the message matches an intent: {"intent":"<id>"}
If no intent clearly matches: {"intent":null}`;
  ...
  try {
    ...
    if (!res.ok) {
      console.error("[classifyIntent] API error:", res.status);
      return null;
    }
    ...
    const cleanText = text.replace(/^```(?:json)?\s*/i, "").replace(/\s*```$/i, "").trim();
    console.log("[classifyIntent]", { message, context, rawText: text, cleanText });

    const parsed = JSON.parse(cleanText);
    const matchedId = parsed?.intent;
    if (!matchedId || typeof matchedId !== "string") return null;

    // Verify the matched ID exists in our intents
    const found = intents.find((i) => i.id === matchedId);
    if (found) return found.id;
    return null;
  } catch (err) {
    console.error("[classifyIntent] parse error:", err);
    return null;
  }
}
 ...
 * @param sessionVariables - Optional session variables to include in LLM context (for open_bot awareness)
 */
export async function callLLMEngine(
  ...
  const varsContext = sessionVariables && Object.keys(sessionVariables).length > 0
    ? `\n\nCurrent conversation data:\n${JSON.stringify(
        Object.fromEntries(
          Object.entries(sessionVariables).filter(([k]) => !k.startsWith("__"))
        )
      )}`
    : "";
  const flowContext = workflowRecord
    ? `\n\n${workflowRecord}${varsContext}`
    : (triggerContext || "") + varsContext;
 ...
 * Implements a multi-turn loop: LLM → tool call → result → LLM → ... → text response.
...
export async function callAgentLLM(params: {
  ...
  while (rounds < maxRounds) {
    rounds++;
    console.log(`[callAgentLLM] Round ${rounds}, messages: ${messages.length}, tools: ${tools.length}, toolNames: ${tools.map(t=>t.name).join(",")}`);
    console.log(`[callAgentLLM] openaiTools sent:`, JSON.stringify(openaiTools.map(t=>({name:t.function.name}))));
    ...
    try {
      ...
      if (!res.ok) {
        ...
        console.error("[callAgentLLM] API error:", res.status, errBody);
        return { response: "Sorry, something went wrong.", toolCalls: allToolCalls };
      }

      const data = await res.json();
      console.log("[callAgentLLM] Raw response:", JSON.stringify(data).substring(0, 500));
      const choice = data.choices?.[0];
      if (!choice) {
        return { response: "No response from AI.", toolCalls: allToolCalls };
      }

      const assistantMsg = choice.message;
      console.log("[callAgentLLM] finish_reason:", choice.finish_reason, "has_tool_calls:", !!assistantMsg.tool_calls, "tool_calls_count:", assistantMsg.tool_calls?.length || 0);
      ...
      if (assistantMsg.tool_calls && assistantMsg.tool_calls.length > 0) {
        ...
        for (const tc of assistantMsg.tool_calls) {
          ...
          console.log(`[callAgentLLM] Tool call: ${fnName}`, JSON.stringify(fnArgs).substring(0, 200));
          ...
          allToolCalls.push({ name: fnName, input: fnArgs, result });
          ...
          console.log(`[callAgentLLM] Tool result: ${fnName}`, JSON.stringify(result).substring(0, 200));
        }
  ...
  console.warn("[callAgentLLM] Max rounds exceeded");
  return { response: "I need a moment to process. Please try again.", toolCalls: allToolCalls };
}
