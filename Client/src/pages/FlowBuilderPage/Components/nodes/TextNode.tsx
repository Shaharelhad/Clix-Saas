...
function TextNode({ data, selected, id }: NodeProps<FlowNode>) {
  ...
  return (
    ...
    >
      ...
      {!data.yesNoMode && data.autoContinue && (
        <span className="text-[10px] text-blue-500 mt-1 block">{t("autoContinue")}</span>
      )}
      {data.allowSkip && !data.yesNoMode && !data.autoContinue && data.expectedReply && (
